  ...
  return (
    <div className='flex flex-col w-full '>
        ...
        <div className='flex flex-col md:flex-row items-start w-full max-h-80 overfow-y-auto '>
            <div className='w-full md:w-2/5 flex flex-col sm:flex-row md:flex-col justify-around md:justify-center items-center p-1  h-fit '>
                <input value={name} onChange={(e)=>setName(e.target.value)} type='text' placeholder="Allergy's name"  className=' bg-healthyGray  rounded-md w-full  text-healthyDarkGray1 px-1'/>
                <button onClick={handleCreation} className='bg-healthyBlue px-2 ml-1 md:ml-0 mt-2 py-1 rounded-md text-sm font-semibold text-white hover:bg-healthyDarkBlue text-center w-full md:w-fit  '>Create allergy</button>
            </div>
            <div className='flex flex-col justify-start items-start md:w-3/5 w-full px-1 '>
                ...
                {food.sort((a, b) => a.name.toUpperCase() < b.name.toUpperCase() ? -1 : 1).map((item,index)=>(<SelectItem key={index} value={item} setSelectedFood={setSelectedFood} selectedFood={selectedFood} />))}
            </div>
        </div>
    </div>
  )
